...
export default function Funeraria() {
  ...
  useEffect(() => {
    const loadItems = async () => {
      try {
        ...
        const loadedItems = await getProductsByTargetScreen(db, "Funeraria");
        setItems(loadedItems);
        console.log("Itens carregados para Funeraria:", loadedItems);
      } catch (error) {
        console.error("Erro ao carregar itens em Funeraria:", error);
        setItems([]);
      }
    };
    loadItems();
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Funerária</Text>
      <FlatList
        data={items}
